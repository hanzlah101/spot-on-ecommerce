...
interface DataTableSkeletonProps extends React.HTMLAttributes<HTMLDivElement> {
   ...
   */
  cellWidths?: { width?: string; height?: string }[]
...
export function DataTableSkeleton(props: DataTableSkeletonProps) {
  const {
    ...
    cellWidths = [{ width: "auto", height: "1.5rem" }],
    withPagination = true,
    shrinkZero = false,
    className,
    ...skeletonProps
  ...
  return (
    <div
      ...
      {...skeletonProps}
    >
      <div className="flex w-full justify-between space-x-2 overflow-auto p-1 md:items-center">
        ...
        {showViewOptions ? (
          <div className="flex items-center gap-x-2">
            <Skeleton className="ml-auto h-7 w-[4.5rem]" />
              ...
              <TableRow key={i} className="hover:bg-transparent">
                {Array.from({ length: columnCount }).map((_, j) => (
                  <TableHead
                    key={j}
                    style={{
                      width: cellWidths[j].width,
                      minWidth: shrinkZero ? cellWidths[j].width : "auto",
                    }}
                  >
                    <Skeleton
                      style={{
                        width: cellWidths[j].width,
                        height: cellWidths[j]?.height ?? "1.5rem",
                      }}
                      className="max-h-6 w-full rounded"
                    />
                  </TableHead>
                ))}
              </TableRow>
